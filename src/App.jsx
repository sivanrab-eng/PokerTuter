...
כמורה, הסבר מה חשוב לחשוב עכשיו ואיך המושג ${concept} רלוונטי לשלב זה.`,
  ...
  const triggerAnalysis = (handGs, handResultData) => {
    ...
    // Build analysis from local logic
    const actions = [...actionLogRef.current].reverse(); // chronological
    const firstAction = actions[0];
    const preflopCtx = analyzeContext(handGs.playerHand, []);
    const finalCtx = analyzeContext(handGs.playerHand, handGs.community);
    const preflopStrength = preflopCtx.strength;

    // 1. First move evaluation
    let firstMoveEval = "";
    if (!firstAction) {
      firstMoveEval = "לא בוצעה פעולה ראשונה לניתוח.";
    } else {
      const a = firstAction.action;
      const handStr = handGs.playerHand.join(", ");
      if (preflopStrength >= 60) {
        if (a === "raise") firstMoveEval = `**${a==="raise"?"ריייז":"קול"} בפרה-פלופ עם ${handStr}** — ✅ בחירה נכונה. עם יד חזקה (${preflopCtx.current.name}) ריייז בונה את הסיר מוקדם וצמצם יריבים.`;
        else if (a === "call") firstMoveEval = `**קול בפרה-פלופ עם ${handStr}** — ❌ פספוס. עם יד חזקה כל כך, **ריייז** היה נכון יותר. קול מאפשר ליריבים להיכנס בזול ויוצר סיכון בפלופ.`;
        else firstMoveEval = `**פולד בפרה-פלופ עם ${handStr}** — ❌ פספוס משמעותי. ${preflopCtx.current.name} זו יד חזקה שכדאי לשחק בה.`;
      } else if (preflopStrength >= 30) {
        if (a === "call") firstMoveEval = `**קול בפרה-פלופ עם ${handStr}** — ✅ סביר. יד בינונית (${preflopCtx.current.name}) מתאימה לקול ולראות את הפלופ בזול.`;
        else if (a === "raise") firstMoveEval = `**ריייז בפרה-פלופ עם ${handStr}** — ⚠️ אגרסיבי. עם יד בינונית עדיף **קול** או **פולד** מחוץ לפוזיציה.`;
        else firstMoveEval = `**פולד בפרה-פלופ עם ${handStr}** — סביר אם הייתה לך אינדיקציה ליד חזקה ביריב. אחרת, אפשר היה לקול.`;
      } else {
        if (a === "fold") firstMoveEval = `**פולד בפרה-פלופ עם ${handStr}** — ✅ בחירה משמעתית. יד חלשה — חיסכון בכסף הוא ניצחון.`;
        else if (a === "call") firstMoveEval = `**קול בפרה-פלופ עם ${handStr}** — ⚠️ ביד חלשה כזו, **פולד** הוא בדרך כלל הצעד הנכון.`;
        else firstMoveEval = `**ריייז בפרה-פלופ עם ${handStr}** — ❌ סיכון גבוה. ביד חלשה זה בלוף שמסתיים לרוב בהפסד.`;
      }
    }
    ...
    if (foldAction) {
      const stageWhereFolded = STAGE_NAMES[foldAction.round] || foldAction.round;
      const strengthAtFold = foldAction.round === "preflop" ? preflopStrength : finalCtx.strength;
      keyPlay = strengthAtFold < 35
        ? `**פולד ב${stageWhereFolded}** — מהלך נכון. הקלפים לא הצדיקו השקעה נוספת. **משמעת היא מיומנות**.`
        : `**פולד ב${stageWhereFolded}** — בעייתי. היד עוד החזיקה (${finalCtx.current.name}). אולי נבהלת מהימור היריב? ${finalCtx.drawFlush||finalCtx.drawStraight?"היו לך גם דראים — סיכוי לשפר.":""}`;
    } else if (raiseAction) {
      const stageRaised = STAGE_NAMES[raiseAction.round] || raiseAction.round;
      keyPlay = preflopStrength >= 50 || finalCtx.strength >= 50
        ? `**ריייז ב${stageRaised}** — אגרסיביות מבוססת. כשיש יד חזקה, ריייז מקבל יותר ערך מהיריב.`
        : `**ריייז ב${stageRaised}** — בלוף. ${finalCtx.drawFlush||finalCtx.drawStraight?"היו לך דראים שתמכו בלחץ.":"בלי בק-אפ של דרא, זה היה מהלך נועז."} בלוף עובד הכי טוב כשהלוח עוזר לטווח שלך.`;
    } else {
      keyPlay = `**שיחקת בעיקר קול** — סגנון פסיבי. כדאי לשלב **ריייזים** כשהיד מצדיקה כדי לבנות סיר ולהפעיל לחץ.`;
    }

    // 3. Summary
    const won = handResultData?.won;
    const tied = handResultData?.tied;
    const folded = handResultData?.type === "fold";
    ...
    // Good point
    if (folded && preflopStrength < 35) goodPoint = "ידעת מתי לפרוש — משמעת בפוקר חשובה לא פחות מאגרסיביות.";
    else if (won) goodPoint = "ניצחת את היד — קראת את המצב נכון והוצאת ערך.";
    else if (raiseAction && preflopStrength >= 50) goodPoint = "השתמשת באגרסיביות נכונה כשהיד הצדיקה זאת.";
    else if (actions.length >= 3) goodPoint = "שיחקת את היד עד הסוף — בנית ניסיון בכל שלב.";
    else goodPoint = "התמדת ולקחת החלטות — כל יד היא הזדמנות ללמוד.";

    // Improve point
    if (preflopStrength >= 60 && firstAction?.action === "call") improvePoint = "עם יד חזקה בפרה-פלופ — תמיד **ריייז**, לא קול.";
    else if (preflopStrength < 25 && firstAction?.action !== "fold") improvePoint = "עם יד חלשה מאוד — **פולד** הוא חבר. חיסכון = רווח.";
    else if (!raiseAction && finalCtx.strength >= 50) improvePoint = "היו לך הזדמנויות לריייז עם יד טובה — אגרסיביות מבוקרת מביאה רווחים.";
    else if (raiseAction && preflopStrength < 30 && finalCtx.strength < 40) improvePoint = "ריייז ביד חלשה הוא בלוף יקר. דאג שיהיה לך **דרא** או יד עם פוטנציאל.";
    else improvePoint = "שים לב לגודל ההימור — תמיד התאם אותו לחוזק היד.";

    // Takeaways (rotate based on what happened)
    if (preflopStrength >= 60) takeaways.push("ידיים פרימיום (AA, KK, QQ, AK) = ריייז כמעט תמיד");
    if (finalCtx.drawFlush) takeaways.push("דרא לפלאש = 9 outs ≈ 35% לסגור עד הריבר");
    if (finalCtx.drawStraight) takeaways.push("דרא פתוח לסטרייט = 8 outs ≈ 31% לסגור");
    if (firstAction?.action === "fold" && preflopStrength < 30) takeaways.push("פולד ביד חלשה הוא ניצחון — חיסכון של כסף");
    if (raiseAction) takeaways.push("ריייז = 2-3x הבלינד בפרה-פלופ, 1/2-2/3 סיר אחרי");
    if (takeaways.length < 3) takeaways.push("פוזיציה = יתרון. שחק יותר ידיים מ-BTN, פחות מ-UTG");
    if (takeaways.length < 3) takeaways.push("Pot Odds: אם הסיכוי לזכות > האחוז שצריך לשלם — קול");

    setAnalysisData({
      firstMove: firstMoveEval,
      keyPlay: keyPlay,
      good: goodPoint,
      improve: improvePoint,
      takeaways: takeaways.slice(0, 3),
      result: won ? "ניצחון" : tied ? "תיקו" : folded ? "פולד" : "הפסד",
      resultColor: won ? "#27ae60" : tied ? "#c9a84c" : "#e74c3c",
    });
...
export default function PokerTutor() {
  ...
  const CARD_MODALS = {
    ...
    botbattle: {
      title:"🤖 קרב בוטים",
      items:[
        {b:"מה זה?", t:"סימולציה אוטומטית של שני בוטים המשחקים זה נגד זה."},
  ...
  if(screen==="menu") return(
    <div style={{height:"100vh",overflow:"hidden",background:"radial-gradient(ellipse at 30% 20%,#0d3320,#061a0e 40%,#030d07 100%)",fontFamily:"Georgia,serif",color:"#d4e8d4",padding:"10px 14px",direction:"rtl",boxSizing:"border-box"}}>
      ...
      {/* Modal */}
      {activeModal && (
        <div onClick={()=>setActiveModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(6px)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn 0.2s ease"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"rgba(10,30,15,0.97)",border:"1px solid rgba(201,168,76,0.5)",borderRadius:16,padding:"18px",maxWidth:360,width:"100%",animation:"slideUp 0.25s ease",direction:"rtl"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div style={{fontSize:14,fontWeight:700,color:"#c9a84c"}}>{CARD_MODALS[activeModal]?.title}</div>
            ...
            <div style={{display:"grid",gap:8}}>
              {CARD_MODALS[activeModal]?.items.map((item,i)=>(
                <div key={i} style={{fontSize:12,lineHeight:1.65,color:"#c8dcc8"}}>
                  <span style={{color:"#c9a84c",fontWeight:700}}>{item.b} </span>{item.t}
                </div>
              ))}
            </div>
            <button onClick={()=>goTo(activeModal)} style={{marginTop:14,width:"100%",padding:"11px",borderRadius:9,border:"none",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:14,fontWeight:700,background:"linear-gradient(135deg,#c9a84c,#8b6914)",color:"#1a1a1a"}}>
              בוא נתחיל ←
            </button>
          </div>
        </div>
